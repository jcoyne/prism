...
# but we want to use the native toolchain here since librubyparser is run
# natively.
if RUBY_ENGINE != "ruby"
  make("build/librubyparser.#{RbConfig::CONFIG["SOEXT"]}")
...
# in CRuby.
unless find_header("prism/extension.h", File.join(__dir__, ".."))
...
# We need to link against the librubyparser.a archive, which is built by the
# project's `Makefile`. We'll build it if it doesn't exist yet, and then add it
# to `mkmf`'s list of local libraries.
archive_target = "build/librubyparser.a"
archive_path = File.expand_path(File.join("../..", archive_target), __dir__)
